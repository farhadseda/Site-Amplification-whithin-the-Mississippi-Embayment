...
start_noise=result(fig).fp-7+4;
ending_noise=start_noise+5;
start_p=result(fig).fp+4;
SP=result(fig).fs-start_p+4;
if SP>5
    ending_p=start_p+5;
else
    ending_p=start_p+SP-1;
end
start_s=result(fig).fs+2;
ending_s=start_s+5;
...
% Using filtfilt (instead of filter) to make the phase of transfer function zero
filtered_N= filtfilt(b,a,result(fig).Time_N(:));
subplot(211);plot(result(fig).t,filtered_N,'Color',[0 0.498039215686275 0])
hold on
start_whole=result(fig).fp-10;
ending_whole=start_whole+150;
start_noise=result(fig).fp-7+4;
